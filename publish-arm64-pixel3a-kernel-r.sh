...
ZIP_FILE=android-arm64-pixel3a-r-4.9-kernelgz-${FILE_TAIL}.zip
...
ZIP_FILE=android-arm64-pixel3a-r-4.9-kernel-${FILE_TAIL}.zip
...
ZIP_FILE=android-arm64-pixel3a-r-4.9-kernel-headers-${FILE_TAIL}.tar.gz
mv ${IMG} ${ZIP_FILE}
gh release upload latest ${ZIP_FILE} --repo ${PUBLISH_REPO} --clobber
...
ZIP_FILE=android-arm64-pixel3a-r-4.9-kernel-uapi-headers-${FILE_TAIL}.tar.gz
mv ${IMG} ${ZIP_FILE}
gh release upload latest ${ZIP_FILE} --repo ${PUBLISH_REPO} --clobber

if [[ -f android-kernel/out/arm64/dist/vmlinux ]]; then
    ZIP_FILE=android-arm64-pixel3a-r-4.9-vmlinux-${FILE_TAIL}.zip
    zip -P qq121212 ${ZIP_FILE} android-kernel/out/arm64/dist/vmlinux
    gh release upload latest ${ZIP_FILE} --repo ${PUBLISH_REPO} --clobber
fi

if [[ -f android-kernel/out/arm64/dist/vmlinux.btf ]]; then
    ZIP_FILE=android-arm64-pixel3a-r-4.9-vmlinux-btf-${FILE_TAIL}.zip
